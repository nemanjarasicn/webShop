import { Component, OnDestroy, OnInit, AfterViewInit, Input, ViewChild, ChangeDetectorRef, EventEmitter, Output, ElementRef } from '@angular/core';
...
export class ListComponent implements OnDestroy, OnInit, AfterViewInit {
  ...
  @Output() triggerAddNewCustom: EventEmitter<any> = new EventEmitter();
  ...
  ngOnInit(): void {
    ...
    if (this.form?.length > 0){
      ...
      this.form.forEach(element => {
        ...
        if (element?.validationPattern) { validators.push(Validators.pattern(element.validationPattern)); }
        ...
        frmGroup[element.key] = (validators.length > 0) ?
          new FormControl({value: element.defaultValue || null, disabled: element?.disabled || false}, validators) :
          new FormControl({value: element.defaultValue || null, disabled: element?.disabled || false});
      });
  ...
  showFullMedia(srcArr: string[]): void{
      this.mediaService.setShowFullMediaActiveArr(srcArr?.length > 0 ? srcArr : []);
  }
